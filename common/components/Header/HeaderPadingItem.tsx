...
export default function HeaderPadingItem({imageString,text,isCurrent,isSoon,buttonText}:HeaderPadingItem) {
  return (
    ...
    isSoon || isCurrent ? "text-font-200": " text-white")
        ...
        <div className={clsx((isSoon || isCurrent)&& "ml-[16px] my-[10px]")}>
            <div className=" py-[4px] pr-[16px] ">
                <button className= {clsx(isSoon || isCurrent ? "rounded-full bg-back-100 w-[50px] h-[24px] text-xs text-font-200 cursor-default" : "hidden")}>{buttonText}</button>  
            </div>
        </div>
    </div>
  )
